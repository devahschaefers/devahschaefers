...
> **How to use this doc:** Check items off (`- [x]`) as they ship. If you add a new idea, give it a fresh checkbox. Keep this file honest — it's the source of truth for what's landed.

> **Current focus: home page only.** The experience/work page is getting a significant overhaul soon, so items scoped to it (marked _[work page — deferred]_) are on hold until after that redesign — no point implementing them against structure that's about to change.

## Micro-interactions (low effort, high charm)

- [x] **#1 Hero name wobble/wave.** On hover of the Instrument Serif "Devah," do a subtle letter-by-letter wave (staggered translateY via `:nth-child` spans). Fits the existing overshoot easing.
- [x] **#2 Tech tag jiggle.** Instead of just color change on `.tech-tag:hover`, add a tiny `rotate(-2deg)` + slight scale — like a sticker you're pressing.
- [x] **#4 Magnetic social icons.** The icons in `Contact.astro` gently pull toward the cursor within ~40px radius. Pair with the existing `translateY(-2px)` hover.
- [x] **#5 Link underline as a squiggle on hover** instead of a straight line — SVG wave drawn with `stroke-dasharray` animation. Ties the brand color in nicely.

## Typography & copy playfulness

- [x] **#6 Rotating role in the hero.** After "I build software, AI systems, and data infrastructure," swap `software` → `LLM agents` → `EEG models` → `satellite pipelines` on a typewriter rhythm. Material for this already exists in `experience.ts`.
- [x] **#7 "Go bears!" interaction.** Click the "go bears!" text → a tiny bear emoji (or bear paw SVG) pops out and drifts up briefly. Pure delight, one-time payoff.
- [ ] **#8 Mixed-case / italic moments** in section labels — keep headings professional but let one word per section lean into Instrument Serif italic, the way "Devah" already does. _(Note: `.serif-italic` class exists in `global.css` but isn't used anywhere yet.)_
- [x] **#9 Time-aware greeting.** "Hi!" becomes "Good morning!" / "Burning the midnight oil?" based on `new Date().getHours()`.
...
_[work page — deferred]_ Hold until the experience-page overhaul lands; these are scoped to cards/layout that will likely change.
...
- [ ] **#19 Scroll-linked accent color shift.** The violet `--color-accent` drifts to a slightly different hue as you scroll down the experience page — keeps things feeling alive without theme whiplash. _[work page — deferred]_
- [ ] **#20 Staggered reveal for experience entries.** `fade-in-d1/d2/d3` already exists — extend this to experience items so they cascade in as you scroll. _[work page — deferred]_
...
1. [x] **#6** rotating role word (literally one line per role already in `experience.ts`)
2. [ ] **#11** DevTools console message
3. [ ] **#18** stat count-up on scroll _[work page — deferred]_
4. [ ] **#14** EEG waveform behind NeuroTech card _[work page — deferred]_
5. [ ] **#24** personality-filled 404

With the work-page items deferred, the next home-page high-leverage plays are **#11** (DevTools console), **#24** (404), and **#3** (cursor-reactive hero tilt) — each ~15–30 minutes and instantly changes how the site *feels*.
